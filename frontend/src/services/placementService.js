...
export const placementService = {
  ...
  async createPlacementDrive(payload) {
    ...
    // Broadcast real-time notification to students
    try {
  ...
  async updatePlacementDrive(driveId, payload) {
    const { data, error } = await supabase
      .from('placements')
      .update(payload)
      .eq('id', driveId)
  ...
  async getEligibleDrivesForStudent(studentProfileId) {
    ...
    return drives.map(d => {
      ...
      const myApp = (d.placement_applications || []).find(app => student && app.student_id === student.id);
  ...
  async applyDrive(studentProfileId, placementId) {
    ...
    try {
      const compName = drive?.companies?.name || 'Company';
  ...
  async getDriveApplicants(placementId) {
