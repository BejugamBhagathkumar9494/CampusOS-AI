...
export const libraryService = {
  ...
  async addBookDirectly(payload: any) {
    try {
  ...
  async returnBook(issuedBookId: string | number) {
  ...
  async getIssuedBooks(studentProfileId?: string) {
    let query = supabase
      .from('issued_books')
  ...
  async requestBookOrPaper(studentProfileId: string, payload: any) {
    const newReq = {
      id: `req-${Date.now()}`,
  ...
  async getBookRequests(studentProfileId: string | null = null) {
    let requests: any[] = [];
    ...
    try {
      ...
      if (rawLocal) {
        ...
        const filtered = studentProfileId ? localList.filter((l: any) => l.student_profile_id === studentProfileId) : localList;
  ...
  async approveBookRequest(requestId: string | number, copiesAvailable: number = 3) {
    let reqObj: any = null;
    ...
    try {
      ...
      if (rawLocal) {
        const localList = JSON.parse(rawLocal);
        const match = localList.find((r: any) => r.id === requestId);
        if (match) {
  ...
  async rejectBookRequest(requestId: string | number) {
    try {
      const rawLocal = localStorage.getItem('local_book_requests');
      if (rawLocal) {
        const localList = JSON.parse(rawLocal);
