...
export const attendanceService = {
  async getStudentAttendance(studentProfileId: string) {
    ...
    try {
      ...
      if (rawLocal) {
        ...
        const filteredLocal = localLogs.filter((l: any) => l.student_id === studentId || l.student_id === studentProfileId);
    ...
    const subjectMap: Record<string, { id: string; code: string; title: string; semester: number; total: number; attended: number }> = {};
  ...
  async getFacultyCoursesWithStudents(facultyProfileId: string) {
    try {
      ...
      if (faculty?.id) console.log('Faculty ID found:', faculty.id);
    } catch (e) {
    ...
    try {
      ...
      if (studentProfiles && studentProfiles.length > 0) {
        studentProfiles.forEach((prof: any, idx: number) => {
    ...
    for (const course of activeCourses) {
      ...
      for (const st of registeredStudents) {
        ...
        const matchedLocal = localLogs.filter((l: any) => l.student_id === st.id && l.course_id === course.id);
        const combinedLogs = [...attLogs, ...matchedLocal];

        const totalCls = combinedLogs.length;
        const attendedCls = combinedLogs.filter((a: any) => a.status === 'present' || a.status === 'late').length;
  ...
  async getCourseAttendanceLogsForDate(courseId: string | number, dateStr: string) {
    ...
    try {
      ...
      if (rawLocal) {
        const localLogs = JSON.parse(rawLocal);
        localLogs.filter((l: any) => l.course_id === courseId && l.date === dateStr).forEach((l: any) => {
  ...
  async saveFacultyAttendance(records: any[]) {
    ...
    try {
      ...
      existingLocal = existingLocal.filter((l: any) => !(l.course_id === sample.course_id && l.date === sample.date));
    ...
    try {
      ...
      if (allStudents && allStudents.length > 0) {
        ...
        for (const st of allStudents) {
          ...
          const attended = attLogs?.filter((a: any) => a.status === 'present' || a.status === 'late').length || 0;
