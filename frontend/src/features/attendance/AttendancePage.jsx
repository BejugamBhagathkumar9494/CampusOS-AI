...
function FacultyAttendancePortal({ profile }) {
  ...
  const handleMarkAll = (status) => {
    ...
    activeRoster.students.forEach(st => {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      setSaving(true);
      setFeedback(null);
      const recordsToSubmit = activeRoster.students.map(st => ({
  ...
  const filteredStudents = (activeRoster?.students || []).filter(st =>
  ...
  const presentCount = filteredStudents.filter(st => (studentStatuses[st.student_id] || 'present') === 'present' || studentStatuses[st.student_id] === 'late').length;
  const absentCount = filteredStudents.filter(st => studentStatuses[st.student_id] === 'absent').length;
  const totalCount = filteredStudents.length;
  const sessionRate = totalCount > 0 ? ((presentCount / totalCount) * 100).toFixed(1) : '100.0';

  const lowAttendanceStudents = (activeRoster?.students || []).filter(st => st.course_attendance_rate < 75.0);
      ...
      {lowAttendanceStudents.length > 0 && (
        <div className="p-4 rounded-2xl bg-amber-50/90 border border-amber-200 text-amber-900 text-xs font-medium flex items-center justify-between gap-4">
          ...
          <div className="flex -space-x-2 shrink-0">
            {lowAttendanceStudents.slice(0, 4).map((st, idx) => (
...
function StudentAttendanceBoard({ profile }) {
