...
import { Sparkles, TrendingUp, Users, Award, BookOpen, AlertTriangle, ShieldCheck, CheckCircle2 } from 'lucide-react';
  ...
  async function loadLiveDbInsights() {
    try {
      ...
      (placementDrives || []).forEach(d => {
        ...
        totalOffered += apps.filter(a => a.status === 'offered' || a.status === 'shortlisted').length;
      });
      const placementForecast = totalApps > 0 ? ((totalOffered / totalApps) * 100).toFixed(1) : '88.5';

      const cgpaSum = (studentRecords || []).reduce((acc, curr) => acc + Number(curr.cgpa || 8.0), 0);
      const avgCgpa = studentRecords?.length ? (cgpaSum / studentRecords.length).toFixed(2) : '8.24';

      const feePaid = (feesData || []).filter(f => f.status === 'paid').reduce((acc, curr) => acc + Number(curr.amount), 0);
      const feePending = (feesData || []).filter(f => f.status === 'pending' || f.status === 'overdue').reduce((acc, curr) => acc + Number(curr.amount), 0);
