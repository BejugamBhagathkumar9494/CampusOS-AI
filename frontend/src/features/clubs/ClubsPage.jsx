...
import { Users, Plus, Trash2, Check, Sparkles, X } from 'lucide-react';
...
export default function ClubsPage() {
  ...
  const handleCreateClub = async (e) => {
    ...
    try {
      setSubmitting(true);
      await eventService.createClub(newClub);
    ...
    <div className="space-y-7 animate-fade-in font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {isAdmin && (
          <button
            ...
            className="px-5 py-2.5 rounded-2xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-lg shadow-purple-500/20 transition-all flex items-center gap-2"
          >
      ...
      {loading ? (
        ...
        <div className="p-12 text-center rounded-2xl bg-slate-50 border border-dashed border-slate-200 space-y-3">
          ...
          <p className="text-xs text-slate-500">There are currently no active student societies registered in the database.</p>
        </div>
      ) : (
              ...
              <div key={club.id} className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm space-y-4 flex flex-col justify-between relative group hover:shadow-md transition-all">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-slate-900 mt-1">{club.name}</h3>
