    ...
    try {
      ...
      if (currentMode === 'llm') {
        ...
        if (qLow.includes('operating system') || qLow.includes('os')) {
          fallbackAnswer = `An **Operating System (OS)** is system software that controls computer hardware and manages software execution.\n\n**Why it matters:**\nIt handles CPU process scheduling, memory allocation, and file storage so programs can run smoothly without conflicting with each other.\n\n**Short Example:**\nWhen you open a browser, the OS allocates RAM, grants CPU execution cycles, and frees resources when closed.\n\n\`\`\`c\n// Process creation using fork()\n#include <stdio.h>\n#include <unistd.h>\n\nint main() {\n    pid_t pid = fork();\n    if (pid == 0) printf("Child process running\\n");\n    else printf("Parent process running\\n");\n    return 0;\n}\n\`\`\``;
        } else if (qLow.includes('binary search') || qLow.includes('algorithm')) {
          fallbackAnswer = `**Binary Search** is an efficient search algorithm that finds an item's position in a sorted list by repeatedly halving the search space.\n\n**Why it matters:**\nInstead of checking every element sequentially in O(N) time, Binary Search finds elements in logarithmic O(log N) time.\n\n**Short Example:**\n\`\`\`python\ndef binary_search(arr, target):\n    left, right = 0, len(arr) - 1\n    while left <= right:\n        mid = (left + right) // 2\n        if arr[mid] == target:\n            return mid\n        elif arr[mid] < target:\n            left = mid + 1\n        else:\n            right = mid - 1\n    return -1\n\`\`\``;
        } else {
          fallbackAnswer = `**${query.title()}**:\n\n**Definition:**\nThis concept is a core subject area in academic computer science and software development.\n\n**Why it matters:**\nUnderstanding it helps you write clean, efficient code, excel in university exams, and answer technical interview questions confidently.\n\n**Short Example:**\nWhen implemented in real applications, it improves system reliability and performance.`;
        }
      } else {
